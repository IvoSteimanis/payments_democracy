*-------------------------------------------------------------------------------------------------------
* SCRIPT:  01_clean.do
* PURPOSE: Clean the raw leader and villager files and merge them into one participant-level dataset.
*
* READS:   data/leader_raw.dta       64 leaders  (32 traditional authorities, 32 elected chairpersons)
*          data/villagers_raw.dta    384 villagers (12 per village)
*
* WRITES:  processed/leader_clean.dta
*          processed/villager_clean.dta
*          processed/data_clean.dta   leaders and villagers appended, N = 448
*
* STRUCTURE:
*   1) Leader data   - identifiers, modules A-G, rule and allocation choices
*   2) Villager data - identifiers, modules A-D, votes
*   3) Merge         - append villagers onto leaders, harmonise labels
*
* Village identifiers and the coded motivation variable arrive pre-computed in the
* released raw data; see the anonymization notes below and in the README.
*
...
*** Village identifier ***
/* ANONYMIZATION: village arrives pre-coded as a numeric ID (1-32) in the released
   raw data. Village names were removed before publication to protect participants:
   with one traditional authority and one elected chairperson per named village, a
   named village would uniquely identify both leaders. The same coding is used in the
   leader and villager datasets, so village fixed effects are unaffected. */
...
*** Day, Month, and Interviewer ***
/* Day, Month, and Interviewer info is only available at village level.
   DEL values are filled from the corresponding TL row within the same village. */
sort village TL
...
*** Interviewer ***
sort village
/* NOTE: Interviewer name columns (Interviewer, BY, CE) removed for anonymization.
   Numeric interviewer codes (Assistant) are retained. */
...
/* NOTE: Name column removed for anonymization */
...
/* ANONYMIZATION: the open-ended religious affiliation answer (A7comment) was
   removed before publication. `religion` ships pre-coded in the released raw
   data using the keyword recode that was applied here. */
...
/* No variation in A8a and A8b, can be dropped for leaders.
   The A8 block (ethnicity, clan) is already removed from the released raw data,
   so the wildcard drop is wrapped in capture and is a no-op in a clean run. */
cap drop A8*
drop A7re
...
/* ANONYMIZATION: the open-ended response to F6 ("Why did you become a leader?")
   was removed before publication. It is replaced by `motivation`, the binary
   intrinsic/extrinsic coding used in the analysis, which ships pre-coded in the
   released raw data. See README, "Data anonymization". */
...
/* ANONYMIZATION: social network nominee names and positions (G1*_position,
   G2_name, G3_name, G4b_name) and kinship relations (G4b_relationship1-6) were
   removed before publication. None enters the analysis. */
rename G4a g4a
recode g4a (2=0)
...
"BF" for Big Five item, "AL" for Authentic Leadership item (self-awareness items were not included in the survey).
...
/* d2 (household income) has extensive missing values — not used in analysis */
...
*** Village identifier ***
/* ANONYMIZATION: village arrives pre-coded as a numeric ID (1-32) in the released
   raw data, using the same coding as the leader dataset so that village fixed
   effects line up across the two files. See the note in the leader section above. */
tab village, gen(village_)

*** Generate unique Villager-ID (65-448, continues from leader IDs 1-64) ***
sort village PNo
...
rename A6 a6
label variable a6 "Since when living in village"
/* ANONYMIZATION: ethnicity (A9) and clan (A9b) were removed before publication.
   Neither enters the analysis. */
***MODULE A DONE***
...
/* NOTE: B3 (WPC chairperson name) removed for anonymization */
...
*---------------------------------------------------
* 3) Append both datasets and create merged sample
*---------------------------------------------------
...
*** Variable to identify all 3 participant groups (DEL, Chiefs, Villagers)
gen villager = 0
