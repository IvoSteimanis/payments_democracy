...
* PURPOSE: Construct the analysis variables and reshape to leader-decision level.
*
* READS:   processed/data_clean.dta
*
* WRITES:  processed/analysis_wide.dta   1 row per participant,      N = 448
*          processed/analysis_long.dta   1 row per leader-decision,  N = 192 (64 leaders x 3 rounds)
*
* BUILDS:  rule choice dummies (dem / fake / dic), baseline rule and leader typology,
*          intrinsic vs extrinsic motivation, Big Five and authentic leadership scores,
*          wealth PCA, villager satisfaction indices, treatment and order indicators.
*
* Missing-value decisions are documented inline at the point where they are applied and
* summarised in the README.
*
* Called by run.do. Expects $working_ANALYSIS to be set.
*-------------------------------------------------------------------------------------------------------
...
/*
	Construct binary indicators dem1-dem6, dic1-dic6, fake1-fake6 from rule
	choices in rounds 1-4 (baseline) and 5-6 (bonus).
	dem7 is the pre-treatment democracy index (mean of dem1-dem4, baseline only).
*/
*democratic
gen dem1=0
...
gen dem5=0
replace dem5=1 if rule5==1
gen dem6=0
replace dem6=1 if rule6==1
...
* Recode rule variables from raw coding (1=dem, 2=dic, 3=fake) to analysis
* coding (1=dem, 2=pseudo-democratic, 3=autocratic) using the dummies above.
...
replace rule3=2 if fake3==1
replace rule3=3 if dic3==1
...
* Indicator for leaders who chose the same rule in baseline (round 1) and both
* bonus rounds (rounds 5 and 6). Captures persistence of rule choice.
gen stable_rule=0
replace stable_rule=1 if rule1==rule5 & rule5==rule6
...
/*
	Based on Frey & Oberholzer (1997) definition:
	  intrinsic (=1): leader mentions personal satisfaction, community service,
	                   or a duty-driven reason for becoming a leader
	  extrinsic (=0): leader accepted the position passively (e.g., appointed,
	                   elected without stated personal motive)

	Coded from the open-ended survey question f6 ("Why did you become a leader?").
	If both intrinsic and extrinsic motives are mentioned, coded as intrinsic (=1)
	since some intrinsic motives may be crowded out by the bonus payment.

	Decision: If f6 is empty (leader gave no answer), coded as extrinsic (=0).
	See line marked with the comment below.
*/

/* ANONYMIZATION: motivation was coded by hand from the open-ended response to F6
   ("Why did you become a leader?"). The verbatim responses are not published, so the
   coded variable ships pre-computed in data/leader_raw.dta and is simply carried
   through the merge here. The coding rule is stated in the comment block above; four
   coders applied it independently (Fleiss kappa 0.87, 88% agreement). Re-running the
   coding requires the confidential response text, which is available from the authors
...
/*
	Missing value treatment: missing responses are recoded to "don't know" (=5),
	which is then mapped to the neutral midpoint (=3) during the scale
	synchronization below (replace ... =3 if `v'==5).

	Satisfaction items are on a 1-5 Likert scale. Scales are synchronized so
	that higher values indicate higher satisfaction across all items.
*/
...
* Reverse-code items where higher raw values indicate lower satisfaction.
* opinionTL_1 (b11b: "TL made promises he didn't keep") and
* opinionTL_7 (b11h: "TL exploits his position") are reverse-coded so that
* higher = less exploitation / fewer broken promises, consistent with other items.
...
egen mean_satisfactionTL = rowmean (e_1 opinionTL_1reverse opinionTL_2 opinionTL_3 opinionTL_4 opinionTL_5 opinionTL_6 opinionTL_7reverse)
...
* -------------------------------------------------------------------
* Drop variables not used in the analysis (03_analysis.do)
* Keeps the dataset lean for the replication package.
* -------------------------------------------------------------------

* Big Five personality (entire battery unused)
cap drop c25 c26 c27 c28 c29 c30 c31 c32 c33 c34
cap drop extraversion agreeableness conscientiousness neuroticism openness
cap drop extraversion_normal agreeableness_normal conscientiousness_normal neuroticism_normal openness_normal

* Authentic Leadership subscales (only authentic_leader1 binary is used)
cap drop c13 c14 c15 c16 c17 c18 c19 c20 c21 c22 c23 c24
cap drop moral_perspective balance_processing relational_transparency
cap drop strong_moral strong_balance strong_relational
cap drop authentic_leader2 AL1 AL_score z_AL_score

* Identity items
cap drop c35a c35b c35c c35d c35e c35f

* Module F: leader survey (unused in analysis)
cap drop f1old f2a f2b f2c f2e f2eold
cap drop f3 f3b f3comment f3bcomment
cap drop f4a f4b f4c f4ccomment f5b
cap drop f7 f10a f10b f10b_new f11
cap drop f12a f12b f12c1 f12c2 f12c3 f12c4 f12d f12e
cap drop f13a f13b f13c f13d
cap drop f18a f18b f18c f18d qa qb
cap drop term elected secretelec competitor dcompetitor

* Module G: social network (unused in analysis)
cap drop g1a g1c g1d g1e g1f g1g g1h
cap drop g1a_position g1c_position g1d_position g1e_position g1f_position g1g_position g1h_position
cap drop g4a g4b_relation1 g4b_relation2 g4b_relation3 g4b_relation4 g4b_relation5 g4b_relation6
cap drop g5a g5c g5d g5e g6 g6comment

* Sociodemographics (unused)
cap drop maritalstatus college hhsize religion
cap drop d1b d2 ssu d4 d5h d6comment d7acomment d7b
cap drop no_educ age_sq
cap drop i_sf i_ob i_em i_p
cap drop z_cattle z_sheeps z_goats quintile pc1

* Villager Module B (unused items)
cap drop b1 b2a b2b b2c b4 b5a b5b b6candidates
cap drop b7a b7b b7c b7d b7e b7f
cap drop b9 b10a b10c b11a b11i b12b

* Villager Module E (relationships, unused)
cap drop e1a e1b e1c e2a e2b

* Game 3 nepotism/trust (unused)
cap drop PSP_RN PSP_NR PSP_NN ASP_RN ASP_NR ASP_NN
cap drop pos_pairRN pos_pairNR pos_pairNN
cap drop decision_A decision_B expec_B positionTG pair

* Satisfaction sub-components (only satisfaction_leader is used)
cap drop e_1 e_2 e_3 e_4 e_5 e_6 e_7 e_8
cap drop opinionTL_1 opinionTL_2 opinionTL_3 opinionTL_4 opinionTL_5 opinionTL_6 opinionTL_7
cap drop opinionTL_1reverse opinionTL_7reverse opinionTL_4reverse
cap drop opinionDEL_1 opinionDEL_2 opinionDEL_3 opinionDEL_4
cap drop mean_satisfactionTL mean_satisfactionDEL
cap drop satisfaction_TL satisfaction_DEL z_satisfaction
cap drop performance_TL performance_DEL performance_leader reverse_e7

* Social type (unused in analysis)
cap drop social_type social_type2

* Leader type dummies (subsumed by leader_type)
cap drop true_democrat pseudo_democrat paternalistic dictator benevolent_dictator

* Other unused
cap drop day_exp month_exp day_pre month_pre interviewer_exp
cap drop expec_game1 relation solved_all 
cap drop villager_preference1 rule_bonus_first rule_bonus_second
cap drop stable_rule pseudo1 pseudo2 pseudo3 pseudo4 pseudo_all
cap drop sum_motivation aux_motivation intrinsic_DEL intrinsic_TL
cap drop group_extrinsic_leader
cap drop a6 a9eth a9bclan

* -------------------------------------------------------------------


/* ANONYMIZATION SAFETY NET: drop any direct identifier or free-text field that may
   have survived an edit upstream. These are removed from the released raw data, so
   in a clean run every drop below is a no-op. Keep this block: it is the last check
   before an identifiable value could reach a published file. */
cap drop Village village_name BX CD Position
cap drop A4bcomment A7comment A8ethnicity A8bclan a9eth a9bclan
cap drop F6_motivation f6 F7_functions f7 F4a F4b f4a f4b F4ccomment f4ccomment
cap drop F3comment f3comment F3bcomment f3bcomment Qacomment F18ccomment F18dcomment
cap drop F10b f10b f10b_new F11comment F12bcomment F12d f12d
cap drop F2d1position F2d2position G6comment D6comment D7acomment
cap drop g1a_position g1c_position g1d_position g1e_position g1f_position g1g_position g1h_position
cap drop g4b_relation1 g4b_relation2 g4b_relation3 g4b_relation4 g4b_relation5 g4b_relation6
cap label drop village
...
/* Rule-choice outcomes expressed in percentage points. The published Figure 5 and
   SOM Tables S11-S12 report coefficients in pp, so the models behind them use these
   rather than the 0/1 dummies. Rescaling the dependent variable of a linear model
   multiplies coefficients and standard errors by 100 and leaves p-values, Wald
   statistics and N unchanged. */
gen dem100  = dem  * 100
gen fake100 = fake * 100
gen dic100  = dic  * 100
label var dem100  "Chose democratic rule (pp)"
label var fake100 "Chose pseudo-democratic rule (pp)"
label var dic100  "Chose autocratic rule (pp)"
...
*interactions
* Drop unused variables from long format (same cleanup as wide)
cap drop stable_rule pseudo1 pseudo2 pseudo3 pseudo4 pseudo_all
cap drop sum_motivation aux_motivation intrinsic_DEL intrinsic_TL
cap drop group_extrinsic_leader
